...
let obrot: number = Math.floor(Math.random() * 359)+1;
let licznik: number = obrot;
let number: Number = obrot + 3240;
let liczba = 0
const winings: number[] = [5,4,3,2,1,0,14,13,12,11,10,9,8,7,6,5]
const messageWinner = document.getElementById('winner');
const czerwony: string = "czerwony"
const czarny: string = "czarny"
const zielony: string = "zielony"
let win: Number = 0
let reszta: Number = 0
let betAmmount: Number = 0
let moneyGot: Number = 0
let choosedNumber: Number = 0
...
let betColor: Array<string> = []
let isBetedRed: Boolean = false
let isBetedBlack: Boolean = false
let isBetedGreen: Boolean = false
const crashClick = document.getElementById("crash");
const betAlert = document.getElementById("betAlert");
const timer = document.getElementById("timer");
let stupidCounter: number = 0


let userName: string = "User"
let betUserName = (document.getElementById("betUserName")) as HTMLElement;;

let betUserAmmount = document.querySelectorAll('#betUserAmmount') as unknown | HTMLElement;
let betAmmountRed: number = 0 
let betAmmountGreen: number = 0 
let betAmmountBlack: number = 0 
...
buttons.forEach( button =>{
    button.addEventListener('click', (e) =>{
        wartosc = Number((e.target as HTMLInputElement).innerText);
...
function spin()
{
    (document.querySelector(".game-cointainer") as HTMLInputElement).style.transform = "rotate("+ number + "deg)";
    wygrana();
    newRandom();    
}
function wygrana(){
